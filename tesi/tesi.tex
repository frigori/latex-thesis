...
{\Huge{\bf Rilevamento del Disturbo dello Spettro Autistico}}\\
\vspace{3mm}
{\Huge{\bf nei Bambini}}\\
\vspace{3mm}
{\Huge{\bf con Dati di Eye Tracking}}\\
\vspace{3mm}
...
{\Large{\bf Relatore: \\ Chiar.mo Prof.\\ Marco Patella}}
...
{\Large{\bf Presentata da: \\ Mattia Giovannini}}
\end{minipage}
...
\pagestyle{plain}
\setcounter{page}{1}
\section{Contesto: l'importanza della diagnosi precoce nell'ASD}
\section{Il problema dei metodi diagnostici tradizionali (spesso lunghi e basati sull'osservazione comportamentale)}
\section{Obiettivo e struttura della tesi}
\chapter{Biomarcatori Visivi e Eye-Tracking nell'Autismo}
\section{Alterazioni dell'attenzione visiva nell'ASD (perché i bambini autistici guardano in modo diverso)}
\section{La tecnologia Eye-Tracking: metriche fondamentali (Fissazioni, Saccadi, Dilatazione pupillare)}
\section{Lo Scanpath (percorso visivo) come biomarcatore oggettivo}
\section{Tassonomia e strutturazione dei dati visivi (Gaze Data Clustering)}
\chapter{Machine Learning per la Classificazione Medica}
\section{Cos'è la Classificazione nel Machine Learning}
\section{Algoritmi tradizionali supervisionati (SVM, Random Forest e il caso pratico del KNN)}
\section{Dal Machine Learning al Deep Learning: l'uso delle Reti Neurali per l'analisi visiva}
\section{Metriche di valutazione dei modelli (Accuratezza, Sensibilità, Specificità)}
\chapter{Stato dell'Arte: ML e Eye-Tracking nell'ASD}
\section{Evoluzione della ricerca sull'IA applicata all'autismo}
\section{Meta-analisi dei risultati (2023-2026): dataset, stimoli visivi e performance medie}
\section{Il problema dei dataset: mancanza di standardizzazione e rischio di overfitting}
\chapter{Studio Sperimentale: Classificazione dell'ASD basata su coordinate spaziali (Caso Studio)}
\section{Contesto del tirocinio e Dataset}
\section{Architettura della Pipeline a due stadi}
\subsection{Fase 1 (KNN)}
\subsection{Fase 2 (Meta-Classificazione)}
\section{Metriche di Validazione}
\section{Analisi dei Risultati}
\chapter{Applicazioni Cliniche e Sviluppi Futuri}
\section{Sistemi oggettivi per lo Screening Precoce (Early Screening Systems)}
\section{Approcci ecologici: l'integrazione con i Serious Games e la valutazione delle funzioni esecutive}
\section{Sfide etiche, privacy dei dati e accettabilità clinica (L'IA come supporto al medico, non come sostituto)}
\chapter{Conclusioni}
\chapter{Bibliografia}
\begin{comment}
Questo studio esplora l’uso di dati di eye-tracking per distinguere tra bambini con e senza Disturbo dello Spettro Autistico (ASD). La classificazione si basa sulle coordinate spaziali dello sguardo (X, Y), utilizzando sia metodi di visualizzazione sia tecniche di apprendimento automatico. L’obiettivo è indagare se i soli pattern di sguardo possano essere predittivi di tratti associati all’ASD.
\section{Contesto e Motivazione}
Aumento globale delle diagnosi di ASD e sfide dei metodi diagnostici tradizionali (basati sull'osservazione comportamentale, spesso soggettivi e dispendiosi in termini di tempo)
\section{Il ruolo dei biomarcatori oggettivi}
L'importanza della diagnosi precoce (es. in età prescolare) e l'uso di tecnologie non invasive
\section{Obiettivi della tesi}
Dimostrare come l'integrazione tra i dati di Eye Tracking e gli algoritmi di Intelligenza Artificiale possa fungere da strumento di screening rapido e affidabile
\section{Struttura dell'elaborato}
Breve riassunto dei capitoli successivi.

\chapter{Background Clinico e Tecnologico}
\section{Il Disturbo dello Spettro Autistico (ASD)}
Definizione clinica, sintomi principali (deficit di interazione sociale e comunicazione) e criteri diagnostici (es. DSM-5, ADOS, CARS)
\section{L'attenzione visiva nell'ASD}
Differenze nei pattern di sguardo (minore attenzione ai volti/stimoli sociali, maggiore attenzione agli oggetti o ai modelli geometrici)
\section{La tecnologia di Eye Tracking (ET)}
\subsection{Principi di funzionamento}
\subsection{Metriche principali}
\section{Introduzione al Machine Learning e Deep Learning}
Panoramica sugli algoritmi supervisionati e sulle reti neurali per il riconoscimento di pattern

\chapter{Stato dell'Arte}
\section{L'applicazione del ML/DL ai dati di Eye Tracking per l'ASD}
Revisione degli studi recenti e dei tassi di accuratezza (variabili dal 55.5\% al 99\%)
\section{Impatto della tipologia di stimoli}
Analisi delle differenze di accuratezza in base agli stimoli utilizzati (stimoli sociali vs. non sociali; statici come immagini vs. dinamici come video; attivi vs. passivi)
\section{Modelli di classificazione utilizzati in letteratura}
Confronto tra metodi tradizionali (SVM, Random Forest, KNN) e approcci di Deep Learning (CNN, LSTM, Reti Ibride CNN-LSTM)

\chapter{Metodologia e Preparazione dei Dati}
\section{Descrizione del Dataset}
Raccolta dati da bambini con diagnosi di ASD (es. tramite CARS) e bambini a sviluppo tipico (TD). Analisi demografica (età, sesso)
\section{Pre-processing dei Dati}
Gestione dei dati mancanti (NaN), del rumore (es. battiti di ciglia) e dei movimenti rapidi (filtri gaussiani, DBScan per la classificazione dei cluster)
Codifica delle variabili e bilanciamento delle classi (Data Augmentation)
\section{Estrazione e Selezione delle Features (Feature Engineering)}
Riduzione della dimensionalità tramite PCA (Principal Component Analysis) o Mutual Information
Trasformazione degli Scanpath in rappresentazioni visive 2D o estrazione di coordinate spaziali (X, Y) e dinamiche temporali (velocità, accelerazione)

\chapter{Sviluppo del Modello di Classificazione}
\section{Metriche di Valutazione}
\section{Prestazioni dei modelli a confronto}
\section{Analisi visiva dei risultati}
